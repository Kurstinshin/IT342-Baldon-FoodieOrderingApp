        ...
        viewModelScope.launch {
            try {
                ApiClient.service.addFood(
                    edu.cit.baldon.foodiemobile.shared.model.FoodRequest(name, desc, price, cat, img)
                )
                fetchFoods()
                onDone()
        ...
        viewModelScope.launch {
            try {
                ApiClient.service.updateFood(
                    id, edu.cit.baldon.foodiemobile.shared.model.FoodRequest(name, desc, price, cat, img)
                )
                fetchFoods()
                onDone()

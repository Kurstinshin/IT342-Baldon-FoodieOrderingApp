...
public class OrderController {
    ...
    @PostMapping("/{userId}")
    @Transactional
    public ResponseEntity<ApiResponse<Order>> placeOrder(@PathVariable Long userId, @RequestBody OrderRequest request) {
        User user = userRepository.findById(userId).orElse(null);
        if (user == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(createResponse(null, "User not found"));
        }

        List<CartItem> cartItems = cartItemRepository.findByUserId(userId);
        if (cartItems.isEmpty()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(createResponse(null, "Cart is empty"));
        }
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(createResponse(savedOrder, "Order placed successfully"));
    }
